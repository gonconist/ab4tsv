...
class AB4TSV(BertPreTrainedModel):
    ...
    This model returns a representation for [tgt], [ctx], [def], [hyps] and [desrc] which serve
    as input to the CNN-Analogy Classifier:
    ...
   ...
   '''
    def __init__(self, config, permutation_invariance=False):
        ...
        self.linear = nn.Linear(64 * (self.emb_size // 2), 1)  # 64*emb_size//2 works but not sure it's ok
